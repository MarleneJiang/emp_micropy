...
# BOOT_MODE = _const()
BOOT_MODE_WITH_NOTHING = 0
BOOT_MODE_WITH_WIFI_STARTUP = 1
BOOT_MODE_EASY_DEVELOP = 2

BOOT_MODE_WITH_WIFI_STARTUP_CODE = '''from emp_wifi import Wifi

if __name__ == '__main__':
    Wifi.connect()'''

BOOT_MODE_EASY_DEVELOP_CODE = '''from emp_wifi import Wifi
...
def set_boot_mode():
    ...
    with open('boot.py', 'w') as f:
        if mode == BOOT_MODE_WITH_NOTHING:
            boot_code = ''
            f.write(boot_code)
            print(rainbow('Boot mode set to WITH_NOTHING', color='green'))
        elif mode == BOOT_MODE_WITH_WIFI_STARTUP:
            boot_code = BOOT_MODE_WITH_WIFI_STARTUP_CODE
            f.write(boot_code)
            print(rainbow('Boot mode set to WITH_WIFI_STARTUP', color='green'))

        elif mode == BOOT_MODE_EASY_DEVELOP:
            config_path()
